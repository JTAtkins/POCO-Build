...
BOOL TestRunnerDlg::OnInitDialog()
{
	...
	GetWindowText(title);
#if defined(_DEBUG)
	title.Append(_T(" [debug]"));
#else
	title.Append(_T("[release]"));
#endif
	SetWindowText(title);

	listCtrl->InsertColumn (0,_T("Type"), LVCFMT_LEFT, 16 + listCtrl->GetStringWidth(_T("Type")), 1);
	listCtrl->InsertColumn (1,_T("Name"), LVCFMT_LEFT, 16 * listCtrl->GetStringWidth(_T("X")), 2);
	listCtrl->InsertColumn (2,_T("Failed Condition"), LVCFMT_LEFT, 24 * listCtrl->GetStringWidth(_T("M")), 3);
	listCtrl->InsertColumn (3,_T("Line"), LVCFMT_LEFT, 16 + listCtrl->GetStringWidth(_T("0000")), 4);
	listCtrl->InsertColumn (4,_T("File Name"), LVCFMT_LEFT, 36 * listCtrl->GetStringWidth (_T("M")), 5);

	int numberOfCases = 0;

	CWinApp* pApp = AfxGetApp();
	CString lastTestCS = pApp->GetProfileString(_T("Tests"), _T("lastTest"));
	std::string lastTest((LPCSTR) lastTestCS.GetString());
	int sel = -1;
	for (std::vector<TestInfo>::iterator it = _tests.begin (); it != _tests.end (); ++it)
	{
		std::string cbName(it->level*4, _T(' '));
		cbName.append(it->pTest->toString());
		comboBox->AddString ((LPCTSTR)cbName.c_str());
		if (sel < 0)
		{
	...
	autoRunBtn->SetCheck(pApp->GetProfileInt(_T("Tests"), _T("autoRun"), BST_UNCHECKED));
...
void TestRunnerDlg::addListEntry(const std::string& type, TestResult *result, Test *test, CppUnitException *e)
{
	...
	_stprintf(stage, _T("%s"), type.c_str ());
    ...
    // Set class string
    listCtrl->SetItemText (currentEntry, 1, (LPCTSTR) test->toString().c_str());

    // Set the asserted text
    listCtrl->SetItemText(currentEntry, 2, (LPCTSTR) e->what());

    // Set the line number
    if (e->lineNumber () == CppUnitException::CPPUNIT_UNKNOWNLINENUMBER)
		_stprintf(stage, _T("<unknown>"));
    else
		_stprintf(stage, _T("%ld"), e->lineNumber ());

    listCtrl->SetItemText(currentEntry, 3, (LPCTSTR) stage);

    // Set the file name
    listCtrl->SetItemText(currentEntry, 4, (LPCTSTR) e->fileName().c_str ());
...
void TestRunnerDlg::updateCountsDisplay ()
{
    ...
    argumentString.Format (_T("%d"), _testsRun);
    statTestsRun    ->SetWindowText (argumentString);

    argumentString.Format (_T("%d"), _errors);
    statErrors      ->SetWindowText (argumentString);

    argumentString.Format (_T("%d"), _failures);
    statFailures    ->SetWindowText (argumentString);

	if (_currentTest)
		argumentString.Format (_T("Execution Time: %3.3lf seconds, Current Test: %s"), (_testEndTime - _testStartTime) / 1000.0, _currentTest->toString().c_str());
	else
		argumentString.Format (_T("Execution Time: %3.3lf seconds"), (_testEndTime - _testStartTime) / 1000.0);
...
void TestRunnerDlg::OnSelchangeComboTest()
{
    ...
    if (currentSelection >= 0 && currentSelection < _tests.size ())
    {
        ...
        beIdle ();
		CWinApp* pApp = AfxGetApp();
		pApp->WriteProfileString(_T("Tests"), _T("lastTest"), (LPCTSTR) _selectedTest->toString().c_str());
    }
    else
    {
        _selectedTest = 0;
...
void TestRunnerDlg::OnBnClickedAutorun()
{
    CButton   *autoRunBtn = (CButton *)GetDlgItem (IDC_CHK_AUTORUN);
	CWinApp* pApp = AfxGetApp();
	pApp->WriteProfileInt(_T("Tests"), _T("autoRun"), autoRunBtn->GetCheck());
}
